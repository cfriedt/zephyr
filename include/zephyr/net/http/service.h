 ...
 *
 * A static HTTP resource is one that is known prior to system initialization. Other resources
 * may be discovered upon system initialization, inserted, or removed by events originating
 * internally or externally to the system.
 *
 * @note The @p resource is the URL without the associated protocol, host, or URL parameters. E.g.
 * the resource for `http://www.foo.com/bar/baz.html#param1=value1` would be `/bar/baz.html`. Every
 * `(service, resource)` pair should be unique.
 *
 ...
 * number will be used and the actual port number assigned will be written back memory (if that
 * memory is writeable).
 *
...
#define HTTP_SERVICE_DEFINE_EMPTY(_name, _host, _port, _concurrent, _backlog, _detail)             \
	__z_http_service_define(_name, _host, _port, _concurrent, _backlog, _detail, 0, 0)
