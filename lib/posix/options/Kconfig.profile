...
# config POSIX_INCLUDE_SYS_FEATURES_H
# 	bool
# 	default y if EVENTFD || GETOPT_LONG || HEAP_MEM_POOL_ADD_SIZE_MQUEUE || POSIX_AEP_REALTIME_CONTROLLER || POSIX_AEP_REALTIME_DEDICATED || POSIX_AEP_REALTIME_MINIMAL || POSIX_API || POSIX_ASYNCHRONOUS_IO || POSIX_BARRIERS || POSIX_BASE_DEFINITIONS || POSIX_C_LIB_EXT || POSIX_CLOCK_SELECTION || POSIX_CPUTIME || POSIX_DEVICE_IO || POSIX_FD_MGMT || POSIX_FILE_SYSTEM || POSIX_FSYNC || POSIX_IPV6 || POSIX_MESSAGE_PASSING || POSIX_MONOTONIC_CLOCK || POSIX_MULTI_PROCESS || POSIX_NETWORKING || POSIX_PAGE_SIZE_BITS || POSIX_PRIORITY_SCHEDULING || POSIX_RAW_SOCKETS || POSIX_READER_WRITER_LOCKS || POSIX_REALTIME_SIGNALS || POSIX_SEMAPHORES || POSIX_SIGNALS || POSIX_SIGNAL_STRING_DESC || POSIX_SINGLE_PROCESS || POSIX_SPIN_LOCKS || POSIX_SYNCHRONIZED_IO || POSIX_SYSCONF_IMPL_FULL || POSIX_SYSCONF_IMPL_MACRO || POSIX_THREAD_ATTR_STACKADDR || POSIX_THREAD_ATTR_STACKSIZE || POSIX_THREAD_CPUTIME || POSIX_THREAD_PRIO_INHERIT || POSIX_THREAD_PRIO_PROTECT || POSIX_THREAD_PRIORITY_SCHEDULING || POSIX_THREADS || POSIX_THREAD_SAFE_FUNCTIONS || POSIX_THREADS_EXT || POSIX_TIMEOUTS || POSIX_TIMERS || POSIX_UNAME_NODENAME_LEN || POSIX_UNAME_VERSION_LEN || TIMER_CREATE_WAIT || XOPEN_STREAMS || XSI_SINGLE_PROCESS || XSI_SYSTEM_LOGGING || XSI_THREADS_EXT
# 	help
# 	  Include zephyr/posix/sys/features.h from autoconf.h which provides feature test macros for POSIX APIs.
