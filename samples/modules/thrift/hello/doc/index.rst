...
Under ``client/``, 4 executables will be generated, and components
...
interchangeably with the ``hello_client`` C++ application, simply to illustrate
the cross-language capabilities of Thrift.
...
In the client terminal, run ``hello_client.py`` app under the host OS:
...
Then, in annother terminal, run the ``hello/client`` sample application
with:
