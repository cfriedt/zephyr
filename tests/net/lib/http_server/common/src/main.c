 ...
 *
 * REST endpoints could be considered as another static resource, as long as
 * the path and handler files are known at compile time.
 */
 ...
 * service which loads a configuration file from the filesystem and loads
 * resources that from a filesystem that are determined at runtime.
 */
