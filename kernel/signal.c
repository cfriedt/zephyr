...
#define K_STRSIGSET_CHARS (MAX(1, DIV_ROUND_UP(CONFIG_SIGNAL_SET_SIZE, BITS_PER_NIBBLE)))
...
static void k_sig_clearbits(struct k_sig_set *result, const struct k_sig_set *set);
static void k_sig_setbits(struct k_sig_set *result, const struct k_sig_set *set2);
static const char *k_strsigset_r(const struct k_sig_set *set, char *buf, size_t len);
static inline void k_sig_dump_fifo(void);
...
static void k_sig_pending_unlocked(struct k_sig_set *set)
{
	...
	/* rather than popping and pushing one entry at a time, peek at the whole buffer */

	for (size_t i = 0, N = ring_buf_peek(&k_sig_fifo, buf, sizeof(buf)); i < N; ++i) {
		struct k_sig_fifo_element *entry;

		entry = &((struct k_sig_fifo_element *)k_sig_slab.buffer)[buf[i]];
...
static int k_sig_match(const struct k_sig_set *match)
{
	int ret = -ENOENT;
	int filter_sig = 0;
	...
	K_SPINLOCK(k_sig_lock) {
		...
		/* populate the set of signals that may be delivered */
		...
		/*
		 * Explicitly deliver realtime signals before asynchronous signals.
		 *
		 * If filter_signo <= 0, any signal will do, otherwise choose only the specified
		 * signal.
		 */
		...
		for (int i = 0, N = ring_buf_size_get(&k_sig_fifo); i < N; ++i) {
			...
			if ((ret < 0) && (entry->tid == tid) && (filter_sig > 0) &&
			    (entry->signo == filter_sig)) {
				/* a signal matching 'filter_sig' was found */
				ret = p;
			} else if ((ret < 0) && (entry->tid == tid) && (filter_sig <= 0) &&
	...
	K_SPINLOCK(k_sig_lock) {
		*entry = (struct k_sig_fifo_element){
		...
		if (IS_ENABLED(CONFIG_SIGNAL_LOG_LEVEL_DBG)) {
			...
			LOG_DBG("%p: pushed signal %d", _current, signo);
			k_sig_pending_unlocked(&pend);
			k_sig_dump_set(&pend, "pending: ");
			k_sig_dump_fifo();
		}
...
int z_impl_k_sig_mask(int how, const struct k_sig_set *set, struct k_sig_set *oset)
{
	...
	switch (how) {
	case K_SIG_BLOCK:
		k_sig_setbits(&tid->base.sig_mask, set);
		break;
	case K_SIG_SETMASK:
		tid->base.sig_mask = *set;
		break;
	case K_SIG_UNBLOCK:
		k_sig_clearbits(&tid->base.sig_mask, set);
		break;
	}
...
{
	const size_t N = ARRAY_SIZE(set->sig);
	...
	if (CONFIG_SIGNAL_SET_SIZE % BITS_PER_LONG == 0) {
		set->sig[N - 1] = -1;
	} else {
		set->sig[N - 1] = BIT(CONFIG_SIGNAL_SET_SIZE % BITS_PER_LONG) - 1;
	}
...
static inline void k_sig_dump_fifo(void)
{
	if (IS_ENABLED(CONFIG_SIGNAL_LOG_LEVEL_DBG)) {
		...
		ring_buf_peek(&k_sig_fifo, buf, N);
		LOG_HEXDUMP_DBG(buf, N, "fifo");
	}
}

static inline void k_sig_dump_set(const struct k_sig_set *set, const char *label)
{
	LOG_DBG("%s%s", label, k_strsigset(set));
}
